...
exp_scaling = pd.read_csv('../data/model_data/pro_exponential.csv')
linear_scaling = pd.read_csv('../data/model_data/pro_linear.csv')
no_scaling = pd.read_csv('../data/model_data/pro_none.csv')
...
def convert_to_weekly_values_pro(df):
    weeks = []
    rep_true_values = []
    dem_true_values = []
    ...
    for week in df.week.unique():
        ...
        rep_true_values.append(pr )
        dem_true_values.append(prd)
...
rep_bots_true = convert_to_weekly_values_pro(rep_bots)
...
dem_bots_true = convert_to_weekly_values_pro(dem_bots)
...
ax.vlines(0.5, -0.05, 0.05, color = 'gray', linestyle = '--', linewidth = 0.5)
ax.vlines(7.5, -0.05, 0.05, color = 'gray', linestyle = '--', linewidth = 0.5)
ax.vlines(12.5, -0.05, 0.05, color = 'gray', linestyle = '--', linewidth = 0.5)
...
fig.savefig('../figures/figure_4C.pdf', bbox_inches = 'tight', dpi = 300, transparent=True)
